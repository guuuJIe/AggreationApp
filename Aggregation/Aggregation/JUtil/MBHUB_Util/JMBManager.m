...
//  SalesMan
//
//  Created by chenqiang on 2018/6/8.
...
#define kGloomyBlackColor  [UIColor colorWithRed:0 green:0 blue:0 alpha:0.8]
...
//        hud.backgroundView.backgroundColor = [UIColor clearColor];
        hud.bezelView.backgroundColor = kGloomyBlackColor;
        hud.label.textColor = UIColorfff;
...
+ (void) showLoadingInView:(UIView *) view{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        hud.label.textColor = UIColorfff;
       [UIActivityIndicatorView appearanceWhenContainedIn:[MBProgressHUD class], nil].color = UIColorfff;
        hud.removeFromSuperViewOnHide = YES;
        gloomyView.frame = view ? CGRectMake(0, 0, view.frame.size.width, view.frame.size.height):
        kDefaultRect;
        if (isShowGloomy) {
...
+(void)hideAlert{
    dispatch_async(dispatch_get_main_queue(), ^{
        MBProgressHUD *hud = [JMBManager HUDForView:gloomyView];
//#if 1
